...
- Gallery: replace `dist/images/live-1.jpg`, `live-2.jpg`, `live-3.jpg`, `live-4.jpg`, and `live-5.jpg`. These are the supplied photos, with orientation corrected and web-sized exports. Update their alt descriptions. Full images open when clicked.
...
2. Replacement: IMG_6873.mov, full recording (58.6 seconds, displayed as 59 seconds). The previous IMG_8318 excerpt is no longer published. The replacement retains its original 568×320 resolution; exported as performance-2-new.mp4 and performance-2-new.webm.
...
Photos and videos were supplied by the band. Original files are not modified. Web exports omit source metadata. The current gallery contains five real photos, with no duplicated stock placeholders. Social art and the band image use an actual video frame. No claims about venue history were added.
...
## Logo and latest media

The circular hero logo is `dist/images/logo.jpg`, exported from the user-supplied Yellow Poo.jpeg. Gallery photos 4 and 5 use IMG_5766.jpeg and IMG_5866.jpeg. Video 2 uses `videos/performance-2-new.mp4` and `.webm`, with `images/performance-2-new.jpg` as its poster. Replace these exact paths when updating this clip. All requested media placeholders are now filled. Original files are untouched.

The repository belongs to theverdicttheband; the former owner remains a write collaborator for maintenance. Use the band-owned URL at the top of this guide.
